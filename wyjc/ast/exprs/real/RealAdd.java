 ...
 */
public class RealAdd extends BinOp<Expr> implements Expr {
	public RealAdd(Expr lhs, Expr rhs, Attribute... attributes) {
		super(lhs,rhs,Types.T_REAL,attributes);
	}

	public RealAdd(Expr lhs, Expr rhs, Collection<Attribute> attributes) {
		super(lhs,rhs,Types.T_REAL,attributes);
	}
	...
	public Expr substitute(Map<String,Expr> binding) {
		...
		return new RealAdd(l,r,attributes());
	}
	...
	public Expr reduce(Map<String, Type> environment) {
		...
		if (l instanceof RealVal && r instanceof RealVal) {
			...
			return new RealVal(i1.value().add(i2.value()), attributes());
		} else {
