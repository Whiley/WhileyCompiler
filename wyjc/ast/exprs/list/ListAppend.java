...
public final class ListAppend extends SyntacticElementImpl implements Expr {
	private Expr source;
	private Expr start;	
	private Expr end;
	
	public ListAppend(Expr source, Expr start, Expr end, Attribute... attributes)  {
		super(attributes);
		this.source = source;
		this.start = start;
		this.end = end;
	...
	public ListAppend(Expr source, Expr start, Expr end, Collection<Attribute> attributes)  {
		super(attributes);
		this.source = source;		
		this.start = start;
		this.end = end;
    ...
    public Expr substitute(Map<String,Expr> binding) {
		...
		return new ListAppend(s,i,j,attributes());
	}
    ...
    public Expr replace(Map<Expr,Expr> binding) {
		...
		if(t != null) {
			...
			return new ListAppend(l,s,e,attributes());
		}
	}
    ...
    public Expr reduce(Map<String, Type> environment) {
		...
		if (s instanceof ListVal && e instanceof IntVal && n instanceof IntVal) {
			...
			if (i.value().compareTo(BigInteger.ZERO) < 0
				...
				return new ListAppend(s, e, n, attributes());
