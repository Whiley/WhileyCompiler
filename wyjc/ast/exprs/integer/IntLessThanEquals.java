 ...
 */
public class IntLessThanEquals extends ConditionBinOp<Expr> implements Condition {
    public IntLessThanEquals(Expr lhs, Expr rhs, Attribute... attributes) {
    	super(lhs,rhs,attributes);		 
    }

    public IntLessThanEquals(Expr lhs, Expr rhs, Collection<Attribute> attributes) {
    	super(lhs,rhs,attributes);
    }
	...
	public Condition substitute(Map<String,Expr> binding) {
		...
		return new IntLessThanEquals(l,r,attributes());
	}
	...
	public Condition reduce(Map<String, Type> environment) {
		...
		if (l instanceof IntVal && r instanceof IntVal) {
			IntVal i1 = (IntVal) l;
			IntVal i2 = (IntVal) r;
			return new BoolVal(i1.value().compareTo(i2.value()) <= 0,
					attributes());
		} else {
	...
	public Pair<WFormula, WFormula> convertCondition(Map<String, Type> environment, ModuleLoader loader) throws ResolveError {
		...
		return new Pair<WFormula,WFormula>(WNumerics.lessThanEq(l.first(),r.first()), constraints );
	}
