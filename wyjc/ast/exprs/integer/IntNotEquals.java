 ...
 */
public class IntNotEquals extends ConditionBinOp<Expr> implements Condition {
	public IntNotEquals(Expr lhs, Expr rhs, Attribute... attributes) {
		super(lhs,rhs,attributes);
	}

	public IntNotEquals(Expr lhs, Expr rhs, Collection<Attribute> attributes) {
		super(lhs,rhs,attributes);
	}
	...
	public Condition substitute(Map<String,Expr> binding) {
		...
		return new IntNotEquals(l,r,attributes());
	}
	...
	public Pair<WFormula, WFormula> convertCondition(Map<String, Type> environment, ModuleLoader loader) throws ResolveError {
		...
		return new Pair<WFormula,WFormula>(WExprs.notEquals(l.first(),r.first()), constraints );
	}
