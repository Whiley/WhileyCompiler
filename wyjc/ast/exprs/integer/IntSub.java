...
public class IntSub extends BinOp<Expr> implements Expr {
	...
	public Expr substitute(Map<String,Expr> binding) {
		...
		return new IntSub(l,r,attributes());
	}
	...
	public Expr reduce(Map<String, Type> environment) {
		...
		if (l instanceof IntVal && r instanceof IntVal) {
			IntVal i1 = (IntVal) l;
			IntVal i2 = (IntVal) r;
			return new IntVal(i1.value().subtract(i2.value()), attributes());
		} else {
	...
	public Triple<WExpr, WFormula, WEnvironment> convert(Map<String, Type> environment, ModuleLoader loader) throws ResolveError {
		...
		wenv.putAll(r.third());		
		return new Triple<WExpr, WFormula, WEnvironment>(subtract(l.first(),r.first()), constraints ,wenv);
	}
