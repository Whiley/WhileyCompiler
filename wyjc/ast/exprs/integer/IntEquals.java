 ...
 */
public class IntEquals extends ConditionBinOp<Expr> implements Condition {
	public IntEquals(Expr lhs, Expr rhs, Attribute... attributes) {
		super(lhs,rhs,attributes);		
	}

	public IntEquals(Expr lhs, Expr rhs, Collection<Attribute> attributes) {
		super(lhs,rhs,attributes);		
	}
	...
	public Condition substitute(Map<String,Expr> binding) {
		...
		return new IntEquals(l,r,attributes());
	}
	...
	public Triple<WFormula, WFormula, WEnvironment> convertCondition(Map<String, Type> environment, ModuleLoader loader) throws ResolveError {
		...
		wenv.addAll(r.third());		
		return new Triple<WFormula, WFormula, WEnvironment>(WExprs.equals(l.first(),r.first()), constraints ,wenv);
	}	
}
