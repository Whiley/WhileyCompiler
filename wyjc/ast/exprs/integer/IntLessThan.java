 ...
 */
public class IntLessThan extends ConditionBinOp<Expr> implements Condition {
	public IntLessThan(Expr lhs, Expr rhs, Attribute... attributes) {
		super(lhs,rhs,attributes);
	}

	public IntLessThan(Expr lhs, Expr rhs, Collection<Attribute> attributes) {
		super(lhs,rhs,attributes);		
	}
	public Condition substitute(Map<String,Expr> binding) {
		Expr l = lhs.substitute(binding);
		Expr r = rhs.substitute(binding);
		return new IntLessThan(l,r,attributes());
	}
	...
	public Triple<WFormula, WFormula, WEnvironment> convertCondition(Map<String, Type> environment, ModuleLoader loader) throws ResolveError {
		...
		wenv.putAll(r.third());		
		return new Triple<WFormula, WFormula, WEnvironment>(WNumerics.lessThan(l.first(),r.first()), constraints ,wenv);
	}		
}
