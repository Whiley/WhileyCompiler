 ...
 */
public class SetNotEquals extends ConditionBinOp<Expr> implements Condition {
	public SetNotEquals(Expr lhs, Expr rhs, Attribute... attributes) {
		super(lhs,rhs,attributes);
	}

	public SetNotEquals(Expr lhs, Expr rhs, Collection<Attribute> attributes) {
		super(lhs,rhs,attributes);
	}		
	...
	public Condition substitute(Map<String,Expr> binding) {
		...
		return new SetNotEquals(l,r,attributes());
	}
	...
	public Condition reduce(Map<String, Type> environment) {
		...
		if (l instanceof SetVal && r instanceof SetVal) {
			SetVal sv1 = (SetVal) l;
			SetVal sv2 = (SetVal) r;			
			return new BoolVal(!sv1.equals(sv2), attributes());
		} else {
