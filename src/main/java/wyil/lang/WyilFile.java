...
public class WyilFile extends AbstractWhileyFile<WyilFile> {
	...
	private static SyntacticItem.Schema[] createSchema() {
		...
		schema[DECL_property] = new Schema(Operands.FOUR,Data.ZERO, "DECL_property") {
			@Override
			public SyntacticItem construct(int opcode, SyntacticItem[] operands, byte[] data) {
				return new Declaration.Property((Tuple<Modifier>) operands[0], (Identifier) operands[1],
						(Tuple<Declaration.Variable>) operands[2], (Tuple<Expr>) operands[3]);
			}
		};
		...
		schema[TYPE_macro] = new Schema(Operands.ONE,Data.ZERO, "TYPE_property") {
			@Override
			public SyntacticItem construct(int opcode, SyntacticItem[] operands, byte[] data) {
				return new Type.Property((Tuple<Type>) operands[0]);
			}
		};
		...
		schema[EXPR_lambdaconst] = new Schema(Operands.ZERO,Data.ZERO, "EXPR_lambdaconst") {
			@Override
			public SyntacticItem construct(int opcode, SyntacticItem[] operands, byte[] data) {
				return null;
			}
		};
		schema[EXPR_lambdainit] = new Schema(Operands.ZERO,Data.ZERO, "EXPR_lambdainit") {
			@Override
			public SyntacticItem construct(int opcode, SyntacticItem[] operands, byte[] data) {
				return null;
			}
