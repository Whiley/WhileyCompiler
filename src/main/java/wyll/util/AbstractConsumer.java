...
public abstract class AbstractConsumer<T> {
	...
	public void visitWyllFile(WyllFile wf, T data) {
...
//	}
	...
	public void visitIfElse(Stmt.IfElse stmt, T data) {
		...
		for(int i=0;i!=branches.size();++i) {
			...
			visitExpression(branch.getFirst(), data);
			visitStatement(branch.getSecond(), data);
		}
		if(stmt.hasDefaultBranch()) {
			visitStatement(stmt.getDefaultBranch(), data);
		}
	}
...
//	}
	...
	public void visitExpression(Expr expr, T data) {
		...
		// Terminals
		case EXPR_nullconstant:
			visitNullConstant((Expr.NullConstant) expr, data);
			break;
		case EXPR_boolconstant:
			visitBoolConstant((Expr.BoolConstant) expr, data);
			break;
		case EXPR_intconstant:
			visitIntConstant((Expr.IntConstant) expr, data);
			break;
		case EXPR_indirectinvoke:
...
//			break;
	...
	public void visitNullConstant(Expr.NullConstant expr, T data) {

	}
	public void visitBoolConstant(Expr.BoolConstant expr, T data) {

	}
	public void visitIntConstant(Expr.IntConstant expr, T data) {
	...
	public void visitFields(Tuple<Decl.Variable> fields, T data) {
		for(int i=0;i!=fields.size();++i) {
			visitField(fields.get(i), data);
		}
	}

	public void visitField(Decl.Variable field, T data) {
