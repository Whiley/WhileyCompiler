...
type class_t is {[ASCII.string] classes, ASCII.string pkg}
...
type MethodIndex is {int op, ASCII.string name, class_t owner, int offset, fun_t type}

type FieldIndex is {int op, ASCII.string name, class_t owner, int offset, jvm_t type}
...
function MethodIndex(int offset, int op, class_t owner, ASCII.string name, fun_t type) -> MethodIndex:
    return {op: op, name: name, owner: owner, offset: offset, type: type}

function FieldIndex(int offset, int op, class_t owner, ASCII.string name, jvm_t type) -> FieldIndex:
    return {op: op, name: name, owner: owner, offset: offset, type: type}

function ConstIndex(int offset, int op, int index) -> ConstIndex:
    return {index: index, op: op, offset: offset}

function code2toString(Bytecode b) -> ASCII.string:
    if b is MethodIndex:
